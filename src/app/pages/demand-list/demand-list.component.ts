...
import { DemandDetail, Demand, DemandTypeDetail, UserDetail, DemandStatusDetail, IDemandSearch, PermissionEnum } from '@/types';
...
@Component({
  ...
  styleUrls: ['./demand-list.style.less'],
})
  ...
  public getDemandList() {
    ...
    this.getDemandList$ = this.demandService.getDemandList(searchData).subscribe(res => {
      if (res.success) {
        ...
        this.message.success('搜索需求列表成功');
      } else {
        this.message.error('搜索需求列表失败');
      }
    });
  }
